...
class SysInfo
{
    ...
    {
        $this->sysInfo = [
            'release' => trim(shell_exec("sw_vers -productVersion") ?? ""),
            'uname-a' => trim(shell_exec("uname -a") ?? ""),
            'hostFQDN' => trim(shell_exec("hostname") ?? ""),
            'hostName' => trim(shell_exec("hostname -s") ?? ""),
            'ipAddr' => trim(shell_exec("ifconfig | grep inet") ?? ""),
            'etcHosts' => trim(shell_exec("cat /etc/hosts") ?? ""),
            'uptime' => trim(shell_exec("uptime") ?? ""),
            'free-mh' => trim(shell_exec("vm_stat") ?? ""),
            'df-h' => trim(shell_exec("df -h") ?? ""),
            'failedServices' => trim(shell_exec("launchctl list | grep -v \"0\"") ?? ""),
            'listeningPorts' => trim(shell_exec("netstat -anp tcp | grep LISTEN | grep -v chrom") ?? ""),
